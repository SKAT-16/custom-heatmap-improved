(function () {
  ...
  if (!showHeatmap) {
    ...
    window.addEventListener('scroll', () => {
      ...
      fetch("http://localhost/custom-heatmap/v2/api/track.php", {
        method: "POST",
    ...
    window.addEventListener('beforeunload', () => {
      ...
      navigator.sendBeacon("http://localhost/custom-heatmap/v2/api/track.php", JSON.stringify({
        session_id: getSessionId(),
        event: "session_end",
  ...
  if (showHeatmap) {
    ...
    script.onload = () => {
      ...
      Object.entries(typeMap).forEach(([type, enabled]) => {
        ...
        fetch(`http://localhost/custom-heatmap/v2/api/heatmap-data.php?url=${encodeURIComponent(urlPath)}&types=${type}`)
          .then(res => res.json())
          .then(data => {
      ...
      summaryBtn.onclick = () => {
        const summaryUrl = `http://localhost/custom-heatmap/v2/api/summary-pdf.php?url=${encodeURIComponent(window.location.pathname)}`;
